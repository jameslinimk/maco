...
export default <Command>{
    ...
    execute: async (interaction) => {
        ...
        const inventoryItems = splitIntoChunks<string>(
            Object.keys(user.inventory).filter(key => user.inventory[key] !== 0).reduce((res, cur) => {
                const item: Item = items[cur]
                res.push(`${item.icon} **${capital(cur)}** - ${item.sellable ? `${formatMoney(item.sellValue)} (Total: ${formatMoney(item.sellValue * user.inventory[cur])})` : " (not sellable)"}\n`)
                return res
            }, [])
            , 7)
