...
export default async (interaction: CommandInteraction) => {
    ...
    if (!search || !search.tracks.length) return interaction.followUp({ content: `\`⛔️\` | No results for **${query}** found!` })
    ...
    pages(splitIntoChunks(tracks, Math.floor(4096 / longestTrackTitle) / 4), interaction, new MessageEmbed()
        ...
        .setFooter({ text: "Please type in chat the number of the song you want to play! ()" })
        , true)
